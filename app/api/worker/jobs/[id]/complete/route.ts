...
export async function POST(_: Request, { params }: { params: { id: string } }) {
  ...
  if (!canTransitionStatus(job.status, JobStatus.COMPLETED, session.role)) {
    return NextResponse.json({ error: 'You can only complete IN_PROGRESS jobs.' }, { status: 400 });
  }

  const updated = await db.job.update({ where: { id: params.id }, data: { status: JobStatus.COMPLETED } });
