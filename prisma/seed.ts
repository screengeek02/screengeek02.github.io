...
async function main() {
  ...
  const workerA = await prisma.user.upsert({
    ...
    update: { name: 'Maria Worker', passwordHash, role: Role.WORKER, workerStatus: 'APPROVED', phone: '555-0104', city: 'Punta Cana', experience: '3 years residential cleaning' },
    create: {
      name: 'Maria Worker',
  ...
  const workerB = await prisma.user.upsert({
    ...
    update: { name: 'Daniel Worker', passwordHash, role: Role.WORKER, workerStatus: 'APPROVED', phone: '555-0105', city: 'Bavaro', experience: '5 years deep cleaning' },
    create: {
      name: 'Daniel Worker',
